...
function loadFile( specificFilePath = "" ) {
    ...
    function callback( text, buffer ) {
        ...
        if( subtitles.length <= 0 ) {
            ...
            let inputs = document.querySelectorAll( "#subtitle-start-form .time-inputs input" );
            loadTimeIntoInputs( inputs, subtitle.start );
            inputs = document.querySelectorAll( "#talking-start-form .time-inputs input" );
            loadTimeIntoInputs( inputs, subtitle.start );
            ...
            inputs = document.querySelectorAll( "#subtitle-end-form .time-inputs input" );
            loadTimeIntoInputs( inputs, subtitle.end );
            inputs = document.querySelectorAll( "#talking-end-form .time-inputs input" );
            loadTimeIntoInputs( inputs, subtitle.end );
...
function loadSubtitlesToForm( form, columnCount, page ) {
    ...
    let totalPages = parseInt( subtitles[subtitles.length - 1].end / MILLISECONDS_PER_PAGE ) + 1;
    ...
    timePeriodDiv.dataset.subtitleTimePeriod = `(${dateToTimeString( new Time( rangeStart ) )}-${dateToTimeString( new Time( rangeEnd ) )})`;
    ...
    for( let i = start; i < subtitles.length; i++ ) {
        ...
        if( subtitle.start > rangeEnd )
            break;

        let startColumn = parseInt( ( subtitle.start.getTime() - rangeStart ) / ( rangeEnd - rangeStart ) * columnCount );
        let endColumn = parseInt( ( subtitle.end.getTime() - rangeStart ) / ( rangeEnd - rangeStart ) * columnCount );
...
function nextSubtitlePage( form ) {
    loadSubtitlesToForm( form, COLUMN_LIMIT, parseInt( form.dataset.subtitlePage ) + 1 );
}
...
function setLastSubtitle() {
    ...
    let talkingTimeInputs = document.querySelectorAll( "#talking-end-form .time-inputs input" );
...
function setNewFileStart() {
    ...
    let newStart = new Time( inputs[0].value, inputs[1].value, inputs[2].value, inputs[3].value );
...
function setNewFileEnd() {
    ...
    let newEnd = new Time( inputs[0].value, inputs[1].value, inputs[2].value, inputs[3].value );

    for( let subtitle of subtitles ) {
        subtitle.start = new Time( scaleLinear( subtitle.start, subtitles[0].start, subtitles[0].start, oldEnd, newEnd ) );
        subtitle.end = new Time( scaleLinear( subtitle.end, subtitles[0].start, subtitles[0].start, oldEnd, newEnd ) );
    }
